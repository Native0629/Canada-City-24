...
export default class AdsScreen extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            subcategory_id: this.params.subCate_id,
            adsData: [],
            status: 1,
            category_name: '',
            subcategory_name: '',
    ...
    getData = async () => {
        var url = getSubcatesUrl + "country_id=" + global.country_id + "&city_id=" + global.city_id + "&category_id=" + this.state.category_id
            + "&subcategory_id=" + this.state.subcategory_id + "&per_page=" + 30 + "&current_page=" + this.state.page + "&location=" + global.currentlocation;
        ...
        fetch(url, {
            ...
            .then((res) => {
                ...
                if (res.status == "200") {
                    ...
                    this.setState({ subcategory_name: global.en_lan ? res.subcategory_en_name : res.subcategory_ar_name });
    ...
    _onRefresh() {
        ...
        var url = getSubcatesUrl + "country_id=" + global.country_id + "&city_id=" + global.city_id + "&category_id=" + this.state.category_id
            + "&subcategory_id=" + this.state.subcategory_id + "&per_page=" + 30 + "&current_page=" + 1;
    ...
    _search() {
        ...
        var url = getSubcatesUrl + "country_id=" + global.country_id + "&city_id=" + global.city_id + "&category_id=" + this.state.category_id
            + "&subcategory_id=" + this.state.subcategory_id + "&per_page=" + 30 + "&current_page=" + this.state.page + "&location=" + this.state.searchKeyword;
        ...
        return (
            <View >
                {/* <View style={styles.callAndLocation}>
                    <TouchableOpacity style={{width: 60, height: 30, borderRadius: 15, backgroundColor: '#426d90', justifyContent: 'center', alignItems: 'center'}}
                                                                        onPress={() => this.phone_call(item.phone_number)}>
                        <Foundation name='telephone' size={15} color="#FFF"/>
                    </TouchableOpacity>
                    <TouchableOpacity style={{width: 60, height: 30, borderRadius: 15, backgroundColor: '#ec2228', justifyContent: 'center', alignItems: 'center'}}
                                            onPress={() => Linking.openURL('https://www.google.com/maps/place/'+ item.adress) } title={item.adress}>
                ...
                <TouchableOpacity style={[styles.featuredFLItem, { width: width(95) }]} onPress={() => this.navigate('AdsDetailScreen', { ads_id: item.id })} >
...

                    {
                        global.en_lan ?
                            <View style={{ height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', paddingHorizontal: 10, borderWidth: 1, borderColor: '#fff' }}>
                                {/* <Text style={{ color: '#8b999f', fontSize: 27, fontFamily: 'cairo-semibold' }}>{global.en_lan? 'Ads' : 'إعلانات'} </Text> */}
                                <Text style={{ color: '#426d90', fontFamily: 'cairo-semibold' }}>{this.state.category_name}  - </Text>
                                <Text style={{ fontFamily: 'cairo-semibold' }}>{this.state.subcategory_name}</Text>
                            </View>

                            :
                            <View style={{ height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', paddingHorizontal: 10, borderWidth: 1, borderColor: '#fff' }}>
                                {/* <Text style={{ color: '#8b999f', fontSize: 27, fontFamily: 'cairo-semibold' }}>{global.en_lan? 'Ads' : 'إعلانات'} </Text> */}
                                <Text style={{ color: '#426d90', fontFamily: 'cairo-semibold' }}>{this.state.subcategory_name}   </Text>
                                <Text style={{ fontFamily: 'cairo-semibold' }}>{this.state.category_name} - </Text>
                            </View>
                    }
                ...
                <View>
                    <View style={{ flexDirection: 'row', height: 50, padding: 5 }}>
                        {global.en_lan ?
                            ...
                            :
                            <View style={styles.searchCon}>
                                <TextInput style={styles.txtInputAr} placeholder="عن ماذا تبحث ؟" value={this.state.searchKeyword} onChangeText={(value) => this.setState({ searchKeyword: value })} />

                                <View style={{ flex: 1, marginHorizontal: 5, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' }}>
                                    <TouchableOpacity onPress={() => this._search()}>
                                        <FontAwesome name='search' size={20} color="#a9adb2" />
                                    </TouchableOpacity>
                                </View>
                    ...
                    {/* { global.en_lan?
                            <View style={{height: 50, flexDirection: 'row', backgroundColor: '#f1f1f1', alignItems: 'center', justifyContent: 'flex-start', paddingHorizontal: 10, borderWidth: 1, borderColor: '#b3b3c2' }}>
                                <Text style={{color: '#426d90', fontFamily: 'cairo-extralight'}}>{this.state.category_name}  - </Text>
                                <Text style={{fontFamily: 'cairo-extralight'}}>{this.state.subcategory_name}</Text>
                            </View>
                            :
                            <View style={{height: 50, flexDirection: 'row', backgroundColor: '#f1f1f1', alignItems: 'center', justifyContent: 'flex-end', paddingHorizontal: 10, borderWidth: 1, borderColor: '#b3b3c2' }}>
                                <Text style={{color: '#426d90', fontFamily: 'cairo-extralight'}}>{this.state.category_name}  - </Text>
                                <Text style={{ fontFamily: 'cairo-extralight'}}>{this.state.subcategory_name}</Text>
                            </View>
                        } */}
                    {
                        this.state.status ?
                            <ImageBackground source={require('../../assets/images/BackgroundImage.png')} style={{ width: '100%', height: '100%', marginTop: 10 }}>
                                ...
                                <FlatList
                                    style={{ marginHorizontal: 10, marginTop: 5, marginBottom: 200 }}
