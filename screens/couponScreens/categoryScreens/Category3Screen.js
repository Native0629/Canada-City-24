...
export default class Category1Screen extends React.Component {
    ...
    componentDidMount(){
        ...
        api.getCoupons(global.country_id, global.city_id, 3).then((res)=>{
                        ...
                        <View style={Style.coupon} key = {index}>
                            <View style={Style.coupon_block} >
                                <TouchableOpacity style={Style.couponLogo}  onPress={()=>this.couponDetail(data)}>
                                    <Image source={{ uri: global.server + data.b_logo}} style={{width: '100%', height:'100%'}} resizeMode="stretch"/>
                                </TouchableOpacity>
                                <View style={Style.couponDescription}>
                                    <View style={{ alignItems: 'flex-start',}}>
                                        <Text style={Style.text_fontOffer} fontSize={16}>{global.en_lan? data.b_name : data.b_name}</Text>
                                        <Text style={Style.text_fontBusinessOffer}>{global.en_lan? data.en_title : data.ar_title}</Text>
                                    </View>
